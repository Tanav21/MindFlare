  ...
  try {
    ...
    if (!tokenString.includes(".") || tokenString.split(".").length !== 3) {
      ...
      return res.status(400).json({
        message:
          "Invalid token format. Expected a JWT ID token from Google Sign-In.",
    ...
    if (!role || !["patient", "doctor"].includes(role)) {
      return res
        .status(400)
        .json({ message: "Valid role (patient or doctor) is required" });
    }
    ...
    // ✅ ONLY allow Google login if email already exists
    let user = await User.findOne({ email });
    ...
    // Attach Google ID if this is first Google login
    if (!user.googleId) {
